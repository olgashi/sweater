...
test('renders NextDaysContainer', () => {
 render(<NextDaysContainer weatherDaily={weatherDataObj.weatherDaily} numDays="5" timezone={weatherDataObj.timezone}/>
)
  const lightRainForecastElement = screen.getAllByText('Light Rain', {exact: false});
  const moderateRainForecastElement = screen.getAllByText('Moderate Rain', {exact: false});
  const overCastCloudsForecastElement = screen.getAllByText('Overcast clouds', {exact: false});
