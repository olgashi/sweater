...
export function getAlerts(weatherData) {
  if (weatherData && weatherData.alerts && Array.isArray(weatherData.alerts)) {
    return weatherData.alerts
      .map((alert) =>
        alert.description ? alert.description : alert.tags.join(", ")
      )
      .filter((el) => el)
      .join(", ");
  }
...
export function determinePercipitation(dayDataObj) {
  ...
  let percipitation = {};
  if (Object.prototype.hasOwnProperty.call(dayDataObj, "snow")) {
    percipitation = {
      type: "Snow",
      value: (parseFloat(dayDataObj.snow) * INCHES_IN_MM).toFixed(2),
    };
  } else if (Object.prototype.hasOwnProperty.call(dayDataObj, "rain")) {
    percipitation = {
      type: "Rain",
      value: (parseFloat(dayDataObj.rain) * INCHES_IN_MM).toFixed(2),
    };
  }
...
export function generateDailyWeatherDataObj(props) {
  if (
    !props ||
    !Object.prototype.hasOwnProperty.call(props, "dayDataObj") ||
    !Object.prototype.hasOwnProperty.call(props, "timezone")
  ) {
    return {};
  }
